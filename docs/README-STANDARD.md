...
3. **Install button row:** one-click install badges, immediately under the badge row.
   ...
   - There is **no one-click web button for Claude**: Claude Code installs via the `claude mcp add` one-liner in Quick start; true one-click for Claude Desktop requires packaging a `.mcpb` Desktop Extension per server (tracked as a follow-up, not a README item).
